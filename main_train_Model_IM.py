...
#CUDA_VISIBLE_DEVICES=0 python  main_train_Model_IM.py   --fold 0   --index 2  &
...
if __name__ == '__main__':
    ...
    logging.info('Load pretrained baseline-EL')
    modelfilename =  'baseline-EL_fold{}_index2_EL.model'.format(args.fold)
    ...
    # optimizer = torch.optim.AdamW(filter(lambda p : p.requires_grad, model.parameters()), lr=LR, weight_decay=0.01) #0.01
    ...
    accuracy,precision,recall,F1_score,AUC_ROC,AUC_PR))
        ...
        # logging.info('BestEpoch={}; Best AUCPR={:.4f}.'.format(
        #     BestEpoch,best_AUCPR
        # ))
